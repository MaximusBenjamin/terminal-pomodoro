...
// LeewayLoadedMsg is sent after leeway is fetched from Supabase.
type LeewayLoadedMsg struct{ Leeway int }
...
// New creates a new settings model, loading from local cache first.
func New(c *api.Client) Model {
	s, _ := Load()
	return Model{client: c, settings: s}
}

// Init loads the leeway from Supabase on startup.
func (m Model) Init() tea.Cmd {
	return func() tea.Msg {
		leeway, err := m.client.GetLeeway()
		if err != nil {
			return LeewayLoadedMsg{Leeway: m.settings.LeewayDaysPerWeek}
		}
		return LeewayLoadedMsg{Leeway: leeway}
	}
}
...
func (m Model) Update(msg tea.Msg) (Model, tea.Cmd) {
	...
	case LeewayLoadedMsg:
		m.settings.LeewayDaysPerWeek = msg.Leeway
		_ = Save(m.settings)
...
func (m *Model) persist() tea.Cmd {
	...
	_ = Save(m.settings)
	return func() tea.Msg {
		if err := m.client.SetLeeway(leeway); err != nil {
...
// Leeway returns the current leeway setting.
